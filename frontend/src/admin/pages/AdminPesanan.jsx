...
export default function AdminPesanan() {
  ...
  const fetchPesanan = async (showSync = false) => {
    ...
    try {
      ...
      if (!isSuperAdmin && dataStores.status === 'success') {
        currentMitraStore = dataStores.data.find(t => t.id === admin?.store_id)
      ...
      if (dataTx.status === 'success') {
        ...
        // ✅ FIX: filter by store_id (bukan nama_toko yang bisa typo)
  ...
  const handleExportCSV = () => {
    const headers = "No. Invoice,Pelanggan,Item,Jumlah,Total Tagihan,Waktu Masuk,Status\n";
    const csvContent = filtered.map(p =>
      `${p.id},"${p.pelanggan}","${p.nama_produk}",${p.items},${p.total},"${p.tanggal}",${p.status}`
    ).join("\n");
  ...
  const filtered = pesananList.filter((p) => {
    ...
    const matchSearch = p.pelanggan.toLowerCase().includes(search.toLowerCase()) || p.id.toLowerCase().includes(search.toLowerCase());
      ...
      <div className="space-y-6 font-sans text-slate-800 animate-in fade-in duration-300">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl bg-amber-50 text-amber-600 flex items-center justify-center border border-amber-100 shrink-0 relative">
                {statsSummary.pendingAction > 0 && <span className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full animate-pulse border-2 border-white"></span>}
                <AlertCircle size={22} />
              </div>
              <div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col lg:flex-row items-start lg:items-center justify-between gap-5">
          <div className="flex items-center gap-3 w-full lg:w-auto">
            ...
            <button onClick={() => fetchPesanan(true)} className="w-10 h-10 bg-white border border-slate-200 rounded-xl flex items-center justify-center text-slate-500 hover:text-emerald-600 hover:border-emerald-200 transition-colors shadow-sm shrink-0">
              <RefreshCw size={18} className={isRefreshing ? 'animate-spin' : ''} />
            </button>
            <button onClick={handleExportCSV} className="w-10 h-10 bg-slate-900 border border-slate-800 rounded-xl flex items-center justify-center text-white hover:bg-slate-800 transition-colors shadow-sm shrink-0 active:scale-95" title="Download Laporan CSV">
              <Download size={18} />
            </button>
          </div>
            ...
            {STATUS_OPTIONS.map((s) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-2 flex-1 lg:flex-none justify-center whitespace-nowrap ${
                  filterStatus === s ? 'bg-white text-slate-900 shadow-sm border border-slate-200' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
                {s}
                      ...
                      <tr key={p.rawId} className={`transition-colors group ${isUrgent ? 'bg-amber-50/30 hover:bg-amber-50/60' : 'hover:bg-slate-50/70'}`}>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            ...
                            {isUrgent && (
                              <button onClick={() => handleStatusChange(p.rawId, 'Dikemas')} className="p-1.5 bg-emerald-100 text-emerald-600 rounded-lg hover:bg-emerald-200 transition-colors" title="Tandai Dikemas">
                                <Zap size={14} />
                              </button>
                            )}
                          </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              ...
              <button onClick={() => setSelectedOrder(null)} className="w-8 h-8 flex items-center justify-center rounded-full bg-white border border-slate-200 text-slate-500 hover:bg-slate-100 hover:text-slate-900 transition-colors">
                <X size={16} />
              </button>
            </div>
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 rounded-xl p-3 border border-slate-100">
                  <p className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-1 mb-1"><User size={12} /> Pelanggan</p>
                  <p className="font-semibold text-sm text-slate-900">{selectedOrder.pelanggan}</p>
                </div>
                <div className="bg-slate-50 rounded-xl p-3 border border-slate-100">
                  <p className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-1 mb-1"><Calendar size={12} /> Tanggal</p>
                  <p className="font-semibold text-sm text-slate-900">{selectedOrder.tanggal}</p>
                </div>
                <div className="col-span-2 bg-slate-50 rounded-xl p-3 border border-slate-100 flex justify-between items-center">
                  <p className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-1"><MapPin size={12} /> Status Pesanan</p>
                  <span className={`text-[10px] font-bold px-2.5 py-1 rounded-md border ${STATUS_CONFIG[selectedOrder.status]?.color || 'bg-slate-100 text-slate-700'}`}>
                    {selectedOrder.status}
                  </span>
                </div>
              </div>
              <div>
                <p className="text-[11px] font-bold text-slate-400 uppercase mb-3 border-b border-slate-100 pb-2">Item Pesanan</p>
                <div className="flex justify-between items-center bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
                  <div className="flex items-center gap-3">
                    <div className="bg-slate-100 text-slate-700 px-2 py-1 rounded-lg text-xs font-bold border border-slate-200">{selectedOrder.items}x</div>
              ...
              <div className="pt-4 border-t border-dashed border-slate-200 flex justify-between items-end">
                <div>
                  <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Metode Pembayaran</p>
                  <p className="text-xs text-slate-600 font-medium">Digital (QRIS / E-Wallet)</p>
                </div>
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50 flex gap-3">
              <button onClick={() => window.print()} className="flex items-center justify-center gap-2 px-4 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl font-bold text-xs hover:bg-slate-100 transition-colors shadow-sm">
                <Printer size={14} /> Cetak Bukti
              </button>
              <button onClick={() => setSelectedOrder(null)} className="flex-1 py-2.5 bg-slate-900 text-white rounded-xl font-bold text-xs hover:bg-black transition-colors shadow-md text-center">
                Tutup Jendela
              </button>
            </div>
          </div>
